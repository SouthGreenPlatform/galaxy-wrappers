...
#        pass
    ...
    if not args.notempf :
        ...
        tabOP=tab.split("/")[-1]+"_out."+ext
        alnN="aln_out_"+ext+".sam"
        fasta1Out=".".join((args.fasta1).split(".")[:-1])
        selectedSeq=fasta1Out.split("/")[-1]+"_selected_"+ext+".fasta"
    ...
    if args.out == None:
        args.out = res2.group(1)+"_out."+ext
    else :
        args.out = args.out.split(".")[0]+"."+ext
...
def checkLoss() :
    ...
    statFile=".".join(args.fasta1.split(".")[:-1]).split("/")[-1]+"_statsLoss"
