...
# guess chrom length
	...
	open(F,">$home/galaxy/static/style/blue/ideogram/$chromfile");
	foreach my $chrom(sort {$a<=>$b} keys(%size_max)){
		my $size = $size_max{$chrom};
		print F "$chrom $size\n";
	}
	close(F);
	$chromosomelength="$base_url/static/style/ideogram/$chromfile";
}
...
foreach my $i (0 .. $#tracks){
	...
	my $sed_cmd = "sed -i \"s/\t/ /g\" \$HOME/galaxy/static/style/blue/ideogram/".$part[$#part];
	system($sed_cmd);
	$iframe .= qq~
