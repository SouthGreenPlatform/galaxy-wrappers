...
my $verbose = 0;
my $pwm_type = 'jaspar';
my $tf_name_select = 'all';
my $second_tf_name_select='';
...
my $dir = getcwd;
#print $dir;
my $numb = int(rand(100000));
my $tmpfoldname=$dir+"/".$numb;
mkdir $tmpfoldname;

my $work_dir = $tmpfoldname."/";
...
if ($ARGV[0]){
    ...
    &Getopt::Std::getopts('a:s:t:g:h:v:o:i:p:w:c:d:', \%opt);# || die ;
}else{
    &help_text; 
}
...
for (my $i=0;$i <= $length;$i++){
	if (($ARGV[$i] =~ /.+\.pfm/)||($ARGV[$i] =~ /.+\.txt/)){
		push (@pwm_files, $ARGV[$i]);
	}
	elsif (exists $ARGV[$i]){
		push (@TF_files, $ARGV[$i]);
	}
}
#print Dumper \@ARGV;
#print Dumper \@pwm_files;
#print Dumper \@TF_files;
...
if($pwm_type eq 'jaspar'){
	...
	my $command = "cat $work_dir/genome/*.tab > $outfile.old";
	&backtick($command);

	open my $in,  '<',  "$outfile.old"     or die "Can't read old file: $!";
	open my $out, '>', $outfile or die "Can't write new file: $!";

	print $out join(";","Name_gene","Name_domain","start","end")."\n";;
	...
	# tar all file
	my $command = "tar -rf $outfile".".tar.gz $work_dir/* ";
	&backtick($command);
...
#     }
...
elsif($pwm_type eq 'place'){
	&backtick("mkdir -p $work_dir");
    # if the pwm_type isn't Jaspar we need to convert them into Jaspar pfm 
    # format and create a matrix_list.txt file. Put new PWM files etc. in 
    # working directory
    # place matrixes all come in a single file 
    
    #if(@ARGV > 1){
	#die "Place matrices should all be in a single file, not ".
    #         join("\n",@ARGV)."\n";
    #}
	...
	# tar all file
	my $command = "tar -rf $outfile $work_dir/* ";
	&backtick($command);
...
#     }
...
elsif($pwm_type eq 'consensus'){
	open(OUT,">".$outfile)or die "failed to open ".$outfile;
	my @results = generate($consensus);
...
elsif($pwm_type eq 'discover'){

	if (-e $db){
	&backtick("mkdir -p $work_dir"."/genome");
	#my $command1 = "cp $db $work_dir"."tar_file";
	#&backtick($command1);	
	my $command2 = "tar -C $work_dir -xf $db";
	&backtick($command2);
	...
	foreach my $chr_file (@chr_files){
		...
		$tab =~ s/fa/tab/;
		$out =~ s/fa/out/;
		my $command = "$moods_mapper -f  $thresh $chr_file $work_dir"."0*/*.pfm > $out";
		#print $command."\n";
		&backtick("$command");
		&parse_out_2_tab($out,$tab,$work_dir."0*/matrix_list.txt",\%file_max_score);
		#&backtick("rm -f $out");
		#&backtick("rm -f $chr_file");
	}
    ...
    die "Can only handle Jaspar,Transfac, Place and DREME formats at present\n";
}
...
sub matrix_min_max{
    ...
    my($matrix_id) = $file =~ /.*([MPCFS][AFNGBHLO]?[0-9]+)\.[1-9].pfm/;
    print $file."\n".$matrix_id."\n";
...
sub parse_matrixdat_2_pfm{
    ...
    if ($pwm_type eq 'place'){
		...
		if($line =~ /^SQ/){			
		#$cpt++;
		my $pfm = &process_matrix_place($ifh,$ofh,$ac,$name,$de,$work_dir);
		push @files, $pfm;
		($de) = "";
		}
		#last if $cpt ==2;
		...
		$cpt++;
		my $pfm = &process_matrix_dreme_red2($ifh,$ofh,$ac,$name,$work_dir);
		push @files, $pfm;
		}		
		#last if $cpt ==2;
		}  
		if ($cpt = 0){
			print "found no motif!";
			exit;
		}  
		else{
			# tar all file
		...
		if($line =~ /^BF\s+(.+)/){($de) .= $1; }
		if($line =~ /^P0/){
		my $pfm = &process_matrix($ifh,$ofh,$ac,$id,$name,$de,$work_dir);
		push @files, $pfm;
		}
		if($line =~ /^\/\//){($de) = ""; }
		}
...
sub generate
{
	...
	for(my $pos = index($str, 'N');
		$pos != -1;
		$pos = index($str, 'N', $pos+1))
	{
		#print("'N' trouvé en position $pos\n");
		$count++;
	}
	if ($count > 7){
		print("$count 'N' trouvés dans $ac.\n");
		next;
	}
	else{
		if ($_[0] =~ /(.*)([RYSWKBDHVNMU])(.*)/) {
...
sub process_matrix_place{
    my($ifh,$ofh,$ac,$name,$de,$work_dir)=@_;
	my $seq;
	
    print $ofh $ac."\t0.0\t".$name."\t".$name." [".$de."]\n";
	...
	if($line =~ /^\/\//){return($work_dir.$ac.'.pfm');}
	
	elsif ($line =~ /^\s+([A-Z]+)/){
	#print Dumper($1);
	
	my @sequences1 = generate($1,$ac);
	#print Dumper(\@sequences1);

	my $patterngen = TFBS::PatternGen::SimplePFM->new(-seq_list=>\@sequences1);
...
sub process_matrix_dreme_red2{
    my($ifh,$ofh,$ac,$name,$work_dir)=@_;
	my $seq;
	
    print $ofh $ac."\t0.0\t".$name."\t".$name."\n";
	
    open(OUT,">".$work_dir.$ac.'.pfm') 
        or die "failed to open ".$work_dir.$ac.'.pfm';
	
	my @sequences1 = generate($name,$ac);
	#print Dumper(\@sequences1);
...
# file 
sub process_matrix{
    my($ifh,$ofh,$ac,$id,$name,$de,$work_dir)=@_;
...
# 	}
    ...
    print OUT join(";",$chr,"",(1),($length+251))."\n" or 
                        die "failed to print to $tab" ;
    print OUT join(";",$chr,"Gene",($length+1),($length+251))."\n" or 
                        die "failed to print to $tab" ;
    ...
    while(my $line = <IN>){
		...
		if($line eq ''){
        ...
        elsif($TF_files[0] eq 'all'){
	    	my($start,$score)= split("\t",$line);
	    	if($score > $score_thresh){	    	
	    		print OUT join(";",$chr,$tf_name,($start+1),($start+$addn))."\n" or 
        	                        die "failed to print to $tab" ;
			...
			#print "Taille tableau:".@TF_files."\n";
			#print Dumper(@TF_files);
			foreach my $tf (@TF_files){
				#print Dumper \$tf;
				if($tf_name =~ /$tf/ix){
					my($start,$score)= split("\t",$line);
					if($score > $score_thresh){	    	
...
sub rev_comp_matrix{
    ...
    my($matrix_id,$version) = $file =~ /.*([MPCFSN][EAFNGBHLO]?[0-9]+).([0-9]*).pfm/;
    print $file."\n".$matrix_id." $version\n";
    unless($matrix_id){die "problem parsing matrix name $file"}
...
sub help_text{
  ...
  pwm_genome_map.pl -g fasta_file -o output_file [options] pwm_file1 [pwm_file2 pwm_file3 ...] TF_name [TF_name2 TF_name3 ...]
			      
                  [-h] for help
                  [-a] <assembly_version> eg GCRh37 as used in full chr. name
                  [-t] <pwm_type> 'jaspar'=default, 'transfac' 'place' 'consensus'                   
                  ...
                  [-c] <consensus_sequence> eg NWDCGAT
                  [-] <> 

exemple command:

convert2jaspar: pwm_genome_map.pl -o output_file -t pwm_type pwm_file
consensus: pwm_genome_map.pl -t consensus -c consensus_sequence -o output file
