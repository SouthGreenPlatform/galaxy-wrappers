...
if($help) { ## display this help when -help is called
	print "\nusage: script.pl [-t matrix file] [-v vcf file] [-c color file] [-p ploidy] [-w number of markers by window] [-s threshold for LOD] [-k window size in K-bases] [-i hybrid name to focus on] [-f file of hybrids to focus on]\n\n";
	...
	print "-f | --focus : file containing several hybrids to focus on (more than one). If -i not indicated (optional).\n";
...
if($hybrid){ ## If focus on hybrid
	...
	open(F11, ">ideogram_hybrid") or die ("Error: can't open output for ideogram\n"); # for a particular hybrid. Output = ideogram format.
	...
	open(FIL, ">len_ideogram_hybrid") or die ("Error : can't open length output for ideogram\n");
	...
	my $blop = 0; # counter. Ideogram.js do not accept names but only numbers.
	...
	if ($indivs){
		...
		for my$ind (@indivs){
			...
			if ($ploidy == 2){
				print F14 "$ind $endS 01\n";
			}
			if ($ploidy == 3){
				print F14 "$ind $endS 012\n";
			}
			if ($ploidy == 4){
				print F14 "$ind $endS 0123\n";
			}
		}
	}
