...
if($help) { ## display this help when -help is called
	...
	print "-v | --vcf : vcf containing the ancestors\n";
	print "-a | --ancestor : ancestor file\n";
	print "-d | --depth : minimal depth for a snp to be used in the analysis\n";
	exit;
}

## check to presence of necessary parameters
if (!$vcf){
	print "You need a vcf file (-t)\n";
	exit;
}   
if (!$ancestors){
	print "You need an ancestors file (-v)\n";
	exit;
} 
...
while (my $li = <F1>){
	chomp($li);
	my@ancestorLi = split(/\t/, $li);
	push(@especesAncestrales, $ancestorLi[0]);
	foreach (my$i = 0; $i<scalar(@ancestorLi); $i++){
		if($i > 0){
			$ancetres{$ancestorLi[$i]} = $ancestorLi[0];
			push(@individusAncestraux, $ancestorLi[$i]);
		}
	}
}
close F1;
...
open(F2,">GSTmatrice") or die ("Error: output wont open\n");
