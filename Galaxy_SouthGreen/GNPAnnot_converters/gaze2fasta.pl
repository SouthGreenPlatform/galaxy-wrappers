...
use lib "/usr/local/bioinfo/galaxy/galaxy_dist/tools/GNPAnnot_converters/lib";
use Bio::SearchIO;
use Bio::Tools::GFF;
use Bio::SeqIO;
...
my ($file_gff,$file_embl,$file_embl_utr,$genomic_seq,$cds_seq,$prot_seq,$file_region) = &gaze2fasta($file_fasta,$file_gaze);

sub eugene2fasta {
    my ($file_fasta,$file_eugene) = @_;
    ...
    my $file_embl     = $file_fasta.".eugene.embl";
    my $file_embl_utr = $file_fasta.".eugene_utr.embl";
    my $genomic_seq   = $file_fasta .".gene.fna";
    my $cds_seq       = $file_fasta .".cds.fna";
    my $prot_seq      = $file_fasta .".prot.faa";
    my $file_gff      = $file_fasta .".gff3";
    my $file_region   = $file_fasta .".region.fna";
    ...
    foreach my $gene_id (sort keys %cds) {
        ...
        for (my $i=0;$i<=$#cds_start;$i++) {
            ...
            my $seq_temp = $seq->subseq( $cds_start[$i],$cds_end[$i]);
        ...
        my $locstr = $location->to_FTstring();
	my %label = (
		     locus_tag            => $gene_id,
		     inference            => $inference,
		     owner                => $species,
	...
	if (@utr5_start) {
	    for (my $k=0;$k<=$#utr5_start;$k++) {
		my $utr_name = $mrna_id . ".5UTR". $k;
		my $feature_utr5_manual =  new Bio::SeqFeature::Generic(
									-seq_id      => $clone_name,
									-source_tag  => "manual_curation",
		...
		$gffio->write_feature($feature_utr5_manual);	
	    }
	}
	if (@utr3_start) {
	    for (my $k=0;$k<=$#utr3_start;$k++) {
		my $utr_name = $mrna_id . ".3UTR". $k;
		my $feature_utr3_manual =  new Bio::SeqFeature::Generic(
									-seq_id      => $clone_name,
									-source_tag  => "manual_curation",
		...
		$gffio->write_feature($feature_utr3_manual);
	    }
	}
    }
    ...
    $file_region = &embl2fna($file_embl,$seq->length,$file_region);
    return ($file_gff,$file_embl,$file_embl_utr,$genomic_seq,$cds_seq,$prot_seq,$file_region);
}
