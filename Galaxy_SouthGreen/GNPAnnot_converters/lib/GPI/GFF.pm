# $Id: GFF.pm,v 1.3 2006/04/11 17:02:59 equevill Exp $
#
...
GPI::GFF - A Bio::SeqAnalysisParserI compliant GFF format parser
...
!!!!! This module is a vulgar copy of the original Bio::Tools::GFF module
!!!!! but as some comment subroutines call because of wrong gff3 formating
!!!!! output. It replace all spaces by '+' and does nt consider the tag 'Note'
!!!!! as a non propriatery tag.
...
=head2 new
 ...
 Title   : new
 Usage   : my $parser = new GPI::GFF(-gff_version => 2,
					         -file        => "filename.gff");
           or
           my $writer = new GPI::GFF(-gff_version => 3,
					         -file        => ">filename.gff3");
...
sub _from_gff3_string {
	...
	$tag             = unescape($tag);
	#Commented to avoid replacement of '+' by spaces. Leave the gff3 as it is
...
# space-separated individual values.
	...
	for my $value ( $feat->each_tag_value($tag) ) {	    
		...
		#Commented to avoid replacement of spaces by '+'
		#$value =~ tr/ /+/;
...
#		    $value = '"' . $value . '"';
		...
		#This leads coma to be replaced with  %2C
		#$value =~ s/([\t\n\=;,])/sprintf("%%%X",ord($1))/ge;
	    } else {
	...
	#Consider the tag 'Note' as well
	$tag= lcfirst($tag) unless ($tag =~/^ID|Name|Alias|Parent|Gap|Target|Note|Dbxref|Ontology_term|Derives_from$/);
...
=head2 newFh
 ...
 Title   : newFh
 Usage   : $fh = GPI::GFF->newFh(-file=>$filename,-format=>'Format')
 Function: does a new() followed by an fh()
 Example : $fh = GPI::GFF->newFh(-file=>$filename,-format=>'Format')
           $feature = <$fh>;            # read a feature object
           print $fh $feature;          # write a feature object
 Returns : filehandle tied to the GPI::GFF class
 Args    :
...
=head2 fh
 ...
 Example : $fh = $obj->fh;      # make a tied filehandle
           $feature = <$fh>;    # read a feature object
           print $fh $feature;  # write a feature object
 Returns : filehandle tied to GPI::GFF class
 Args    : none
