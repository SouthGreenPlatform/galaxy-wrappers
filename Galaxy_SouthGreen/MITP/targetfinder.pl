...
my $current_dir = $ENV{'PWD'};   # Location of current directory, base directory where temporary directory will be created
my $fasta = "fasta36"; # Location of the fasta36 binary

# Creates a temporary directory, unless it exists
my $dir = "$current_dir/tmp";
unless (-e $dir) {
	system "mkdir $current_dir/tmp";  # Note that mkdir must be in your PATH variable
}
...
sub var_check {
	...
	if (!$current_dir) {
		print " ERROR: environmental variable 'PWD' not set\n";
		&var_error();
	}
	if (!defined($fasta)) {
